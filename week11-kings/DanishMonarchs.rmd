...
separator: kings.csv is separated by commas
...
3. How many columns does the object have when created with these different functions? 
 - read.csv and read_csv give the object 11 columns, read.csv2 and read_csv2 only give the object 1 column. This is because they work for semicolon separated values, but the spreadsheet is separated by commas
...
# Where did slice() come from??? A bit of guessing... but it works!
kings %>% 
  filter(!is.na(Start_year)&!is.na(End_year)) %>% 
  mutate(duration=End_year-Start_year) %>%
  filter(duration>19.57692) %>% 
...
# Yeah this is probably too simple to be correct
kings %>% 
  filter(!is.na(Start_year)&!is.na(End_year)) %>% 
  mutate(duration=End_year-Start_year) %>%
...
kings %>% 
  ...
  ggplot(aes(x = Mid_year, y = duration))+geom_point()
kings %>% 
  filter(!is.na(Start_year)&!is.na(End_year)) %>% 
  mutate(duration=End_year-Start_year) %>%
  mutate(Mid_year=Start_year+duration/2) %>% 
  ggplot(aes(x = Mid_year, y = duration))+geom_smooth()
```
