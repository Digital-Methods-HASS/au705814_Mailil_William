...
# make scientific notation go away
options(scipen = 999)
# renaming the labels
names(gapminder)[4] <- "Life_expectancy"
names(gapminder)[6] <- "GDP_per_capita"


ggplot(subset(gapminder, year == 1952), aes(GDP_per_capita, Life_expectancy, size = pop)) +
  geom_point(aes(color=continent)) +
  scale_x_log10() +
  ggtitle("GDP per capita 1952")

# Who is the outlier on the right?
gapminder %>% 
  filter(year==1952,GDP_per_capita>100000)
...
```{r 2007}
ggplot(subset(gapminder, year == 2007), aes(GDP_per_capita, Life_expectancy, size = pop)) +
  geom_point(aes(color=continent)) +
  scale_x_log10() +
  ggtitle("GDP per capita 2007")

# Top 5 richest countries in 2007
gapminder %>% 
  filter(year==2007) %>% 
...
There are exponential differences between the GDP per capita of the countries listed, but not the life expectancy. Especially the outlier in 1952 makes this apparent.
...
```{r anim1 start}
anim <- ggplot(gapminder, aes(GDP_per_capita, Life_expectancy, size = pop)) +
  geom_point(aes(color=continent)) +
...
anim + transition_states(year, 
                      transition_length = 1,
                      state_length = 1)+
   labs(title = "{closest_state}") # should change title label based on year
...
```{r anim2}
anim2 <- ggplot(gapminder, aes(GDP_per_capita, Life_expectancy, size = pop)) +
  geom_point() +
  scale_x_log10() + # convert x to log scale
  transition_time(year)+
  geom_point(aes(color=continent)) +
  labs(title = "{frame_time}") # should change title label based on year
anim2
```
